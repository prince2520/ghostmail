...
const HomeInbox = () => {
    ...
    return (
        <div className="flex flex-col gap-y-4 px py-2 md:px-2 md:py-4 md:px-4 md:py-6 w-full rounded-md border">
            {currMailDetail && currMailDetail.messages && currMailDetail.messages.length > 0 ? <HomeInboxMessages messages={currMailDetail.messages} mailId={currMailDetail.id} /> : <HomeInboxNoMails />}
        </div>
    );
};
